...
export default function Convocatorias({ modoOscuro }: { modoOscuro: boolean }) {
  ...
  const handleUpdate = async (formData: any) => {
    try {
      // 🔹 Igual mapeo pero respetando el `id`
      const conv = {
        title: formData.nombre,
        description: formData.descripcion,
        resources: formData.recursos,
  ...
  const linkColor = modoOscuro
    ? "text-[#39A900] hover:text-[#2d8500]"
    : "text-[#39A900] hover:text-[#2d8500]";
          ...
          >
            ...
            <div className="p-4 md:p-6 space-y-4">
              ...
              >
                ...
                <div className="space-y-2">
                  <p><b>Entidad ID:</b> {conv.institutionId}</p>
                  <p><b>Línea ID:</b> {conv.lineId}</p>
                  <p><b>Público Objetivo ID:</b> {conv.targetAudienceId}</p>
                  <p><b>Área de Interés ID:</b> {conv.interestId}</p>
                  <p><b>Usuario ID:</b> {conv.userId}</p>
                </div>
              </div>
