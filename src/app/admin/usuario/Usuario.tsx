...
export default function Usuario({ modoOscuro }: UsuarioProps) {
  ...
  const usuariosFiltrados = usuarios.filter(
    (u) => u.name.toLowerCase().includes(busqueda.toLowerCase()) || u.email.toLowerCase().includes(busqueda.toLowerCase())
  );
    ...
    <>
      <div className={`rounded-3xl p-10 max-w-9xl mx-auto my-12 ${bgColor} ${textColor} ${borderColor}`}>
        ...
        <div className="text-center mb-10">
          ...
          <p className={`text-lg ${secondaryText}`}>Administra los usuarios del sistema de forma sencilla y eficiente</p>
        </div>
        ...
        <div className="space-y-5">
          {usuariosFiltrados.length === 0 ? (
            <div className={`text-center py-16 rounded-2xl border ${emptyStateBg} ${emptyStateBorder}`}>
              ...
              <button
                onClick={() => setMostrarModalCrear(true)}
                className={`mt-4 px-5 py-2 rounded-lg ${buttonBg} ${buttonText} ${buttonHover}`}
              >
                Crear nuevo usuario
              </button>
            </div>
            ...
            usuariosFiltrados.map((usuario) => (
              <div
                key={usuario.id}
                className={`p-6 rounded-2xl border shadow-md hover:shadow-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-5 ${cardBg} ${borderColor}`}
              >
                <div className="flex items-start gap-4 w-full">
                  ...
                  <div className="flex-1 space-y-2">
                    <h3 className={`text-xl font-semibold ${modoOscuro ? 'text-white' : 'text-gray-800'}`}>{usuario.name}</h3>
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className={`p-3 rounded-xl ${modoOscuro ? 'bg-blue-900/30 text-blue-400' : 'bg-blue-50 text-blue-600'}`}
                  >
                    <Edit size={20} />
                  </button>
                  <button
                    onClick={() => eliminarUsuario(usuario.id)}
                    className={`p-3 rounded-xl ${modoOscuro ? 'bg-red-900/30 text-red-400' : 'bg-red-50 text-red-600'}`}
                  >
                    <Trash2 size={20} />
                  </button>
                </div>
